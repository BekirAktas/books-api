...
import validate from '@/resources/book/book.validation';
...
class BookController implements Controller {
    ...
    private createBook = async (
        ...
        try {
            const { title, author, price, ISBN, language, number_of_pages, publisher } = req.body;

            const book = await this.BookService.create(title, author, price, ISBN, language, number_of_pages, publisher);
    ...
    private showBook = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
    ...
    private updateBook = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            const { title, author, price, ISBN, language, number_of_pages, publisher } = req.body;

            const book = await this.BookService.updateBook(req.params.id, title, author, price, ISBN, language, number_of_pages, publisher);
